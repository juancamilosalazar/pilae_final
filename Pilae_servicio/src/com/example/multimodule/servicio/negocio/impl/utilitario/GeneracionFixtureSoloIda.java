...
public class GeneracionFixtureSoloIda {
    ...
    private static GeneracionFixtureIdaYVuelta.Partido[][] calcularLigaNumEquiposPar(HashMap equipos) {
        int numRondas = equipos.size() - 1;
        int numPartidosPorRonda = equipos.size() / 2;

        GeneracionFixtureIdaYVuelta.Partido[][] rondas = new GeneracionFixtureIdaYVuelta.Partido[numRondas][numPartidosPorRonda];

        for (int i = 0, k = 0; i < numRondas; i++) {
            for (int j = 0; j < numPartidosPorRonda; j++) {
                rondas[i][j] = new GeneracionFixtureIdaYVuelta.Partido();
                rondas[i][j].local = k;
                k++;
    ...
    private static GeneracionFixtureIdaYVuelta.Partido[][] calcularLigaNumEquiposImpar(HashMap equipos) {
    ...
    public GeneracionFixtureIdaYVuelta.Partido[][] calcularLiga(HashMap<Integer, EquipoEntidad> equipos) {
    ...
    public List<PartidoEntidad> mostrarPartidos(GeneracionFixtureIdaYVuelta.Partido[][] rondas, HashMap<Integer, EquipoEntidad> equipos, TorneoEntidad id) {
        ...
        for (int i = 0; i < rondas.length; i++) {
            for (int j = 0; j < rondas[i].length; j++) {
                ...
                partidoEntidad.setIdaVuelta("solo ida");
